...
async function getCategories() {
    ...
    for(let i=0;i<categoriesCount.length;i++) {
        res['categories'].push({ id: categoriesCount[i].category, category: categoriesCount[i].name, count: categoriesCount[i].count });
    }

    for(let i=0;i<subCategories.length;i++) {
        res['subCategories'].push({ id: subCategories[i].id, subname: subCategories[i].subname, categoryId: subCategories[i].categoryId, category: subCategories[i].category, count: subCategories[i].count });
    }
    return res;
}
