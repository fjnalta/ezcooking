...
async function updateDish(recipeName, duration, shortDescription, filename, description, user, category, subCategory, ingredients) {

    // TODO - THIZ
    let unitId = [];

    for(let i=0;i<ingredients.length;i++) {
        unitId[i] = await getUnitId(ingredients[i].Einheit)
    }
    let userId = await getUserId(user);
    let categoryId = await getCategoryId(category);
    let subCategoryId = await getSubCategoryId(subCategory);
...
async function changeUsername(userId, newUsername) {
    try {
        let res = await pool.query({sql: "UPDATE users SET ? WHERE id = ?" }, [{ name: newUsername }, userId]);
        return res;
    } catch (e) {
...
async function changeEmail(userId, newEmail) {
    try {
        let res = await pool.query({sql: "UPDATE users SET ? WHERE id = ?" }, [{ email: newEmail }, userId]);
        return res;
    } catch (e) {
        console.log('Error change username');
        console.log(e);
...
async function changePassword(userId, newPassword) {
    try {
        let res = await pool.query({sql: "UPDATE users SET ? WHERE id = ?" }, [{ password: newPassword }, userId]);
        return res;
    } catch (e) {
...
async function deleteAccount(userId) {
    try {
        ...
        let res = await pool.query({sql: "DELETE FROM users WHERE id = ?" }, userId);
        return res;
    } catch (e) {
