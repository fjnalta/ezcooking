...
async function isUserAdmin(userId) {
    let res;
    try {
        res = await pool.query({sql: "SELECT role FROM users WHERE id = ?"}, userId);
    } catch (e) {
...
async function getSubCategories(category) {
    try {
        return await pool.query({sql: "SELECT category_subcategory.id, subname, category, image_name, cc.name FROM category_subcategory JOIN category_category cc on category_subcategory.category = cc.id WHERE category = ?"}, category);
    } catch (e) {
...
// ========================
async function setDishOwner(userId, dishId) {
    try {
        return await pool.query({sql: "UPDATE dish SET ? WHERE id = ?"}, [{creator_id: userId}, dishId]);
    } catch (e) {
