...
async function getDish(dish) {
    ...
    let resIngredients;
    try {
        resDish = await pool.query({ sql: "SELECT * FROM dish WHERE id = ?" }, dish);
        resIngredients = await pool.query({ sql: "SELECT count, cu.name as unit_name, cu.short_name, i.name FROM dish_ingredients JOIN ingredient i on dish_ingredients.ingredient_id = i.id JOIN category_unit cu on dish_ingredients.count_unit = cu.id WHERE dish_id = ?" }, dish);
    } catch (e) {
...
async function getCategories() {
    // TODO - fix count calculation it's a memory leak!!!
    // TODO - buggy
    // maybe join tables or w/e
    ...
    try {
        categories = await pool.query({ sql: "SELECT * FROM category_category"});
        categoriesCount = await pool.query({ sql: "SELECT category, COUNT(*) AS 'count' FROM dish GROUP BY category" });
        subCategories = await pool.query({ sql: "SELECT * FROM category_subcategory"});
        subCategoriesCount = await pool.query({ sql: "SELECT subcategory, COUNT(*) AS 'count' FROM dish GROUP BY subcategory" });
    } catch (e) {
    ...
    // Bring categories into usable format
    for(let i=0;i<categories.length;i++) {
        ...
        for(let j=0;j<categoriesCount.length;j++) {
            if(res[i].id === categoriesCount[j].category) {
                res[i]['count'] = categoriesCount[j].count;
            }
        }
    }
    ...
    for(let i=0;i<subCategories.length;i++) {
        for(let j=0;j<res.length;j++) {
            // Include SubCategories in categories
            if(subCategories[i]['category'] === res[j].id) {
                res[j]['subCategories'].push({id: subCategories[i].id, name: subCategories[i].subname, count: 0 });
            }
        }
    }

    // Include subCategory count
    for(let i=0;i<res.length;i++) {
        for(let j=0;j<subCategoriesCount.length;j++) {
            for(let k=0;k<res[i]['subCategories'].length;k++) {

                // console.log(res[i]['subCategories'][k].id);
                // console.log(subCategoriesCount[j].subcategory);

                if(res[i]['subCategories'][k].id === subCategoriesCount[j].subcategory) {
                    res[i]['subCategories'][k].count = subCategoriesCount[i].count;
                }
            }
        }
    }
