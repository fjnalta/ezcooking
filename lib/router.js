...
// ADMIN - routes
// ================
router.get('/admin', auth.handleSession, auth.checkUserLogin, auth.checkAdminPermission, (req, res) => {
    res.render('admin', {
        session: req.session,
    });
});

router.get('/dish/:id/assign', auth.handleSession, auth.checkUserLogin, auth.checkAdminPermission, async (req, res) => {
    const data = await dbHandler.setDishOwner(req.session.userId, req.params.id);
    if(data) {
