...
router.get('/dish/:id', handleSession, checkUserPermission, function (req, res) {
    res.render('dish', {
        session: req.session
    });
});
...
router.post('/dish', upload.single('data'), handleSession, checkUserPermission, function (req, res) {
    // TODO - use promises for chained queries
    ...
    // Get UserID
    pool.query({ sql: "SELECT id FROM users WHERE name = ?"}, req.session.username, function (err, result) {
        if (err) {
            throw err;
        } else {
            if (result.length > 0) {
                ...
                // Get Category ID
                pool.query({ sql: "SELECT id FROM category_category WHERE name = ?"}, req.body.category, function (err, result) {
                    if (err) {
                        throw err;
                    } else {
                        if (result.length > 0) {
                            ...
                            // Get SubCategory ID
                            pool.query({ sql: "SELECT id FROM category_subcategory WHERE subname = ?"}, req.body.subCategory, function (err, result) {
                                if (err) {
                                    throw err;
                                } else {
                                    if (result.length > 0) {
                                        ...
                                        // Create Dish
                                        pool.query({ sql: "INSERT INTO dish (id, name, duration, short_description, image_name, preparation, creation_date, creator_id, category, subcategory) VALUES ?" }, records, function (err, result, fields) {
                                            if (err) {
                                                throw err;
                                            } else {
                                                if (result.length > 0) {
                                                    ...
                                                    // search if ingredients are available
    ...
    // Search/Create Ingredients

    // Create DishIngredients with ID from Dish, Ingredients and Units
