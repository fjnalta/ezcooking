...
class DishHandler {
    ...
    browseDish = async (page) => {
        ...
        try {
            res = await this.dbHandler.query({sql: "SELECT * FROM dish LIMIT ?, 9"}, curRes);
        } catch (e) {
            console.log('Error loading dishes for page ' + page);
        }

        return res;
    };

    getNumberOfDishes = async () => {
        let res;
        try {
            res = await this.dbHandler.query({sql: "SELECT COUNT(id) FROM dish"});
        } catch (e) {
            console.log('Error loading number of dishes');
        }
        return res[0]['COUNT(id)'];
    };

    getNumberOfDishesFromCategory = async (category) => {
        let res;
        try {
            res = await this.dbHandler.query({sql: "SELECT COUNT(id) FROM dish_category WHERE category_id = ?"}, category);
        } catch (e) {
            console.log('Error loading number of dishes from category');
        }
        return res[0]['COUNT(id)'];
    }

    getNumberOfDishesFromSubCategory = async (subcategory) => {
        let res;
        try {
            res = await this.dbHandler.query({sql: "SELECT COUNT(id) FROM dish_category WHERE subcategory_id = ?"}, subcategory);
        } catch (e) {
            console.log('Error loading number of dishes from subcategory');
        }
        return res[0]['COUNT(id)'];
    }

    getDishesFromCategory = async (page, category) => {
        let res;
        // handle current page
        let curRes;
        if(page <= 1) {
        ...
        try {
            res = await this.dbHandler.query({sql: "SELECT\n" +
                    ...
                    "LIMIT ?, 9"}, [category, curRes]);
        } catch (e) {
            console.log('Error loading dishes from category for page ' + page);
        }
        return res;
    }
    ...
    getDishesFromSubCategory = async (page, subCategory) => {
        let res = {};
        // handle current page
        let curRes;
        if(page <= 1) {
        ...
        try {
            let category = await this.dbHandler.query({
                sql: "SELECT subname as subcategory,\n" +
                    "category_subcategory.image_name,\n" +
                    "cc.name as category\n" +
            ...
            let dish = await this.dbHandler.query({
                sql:
                    ...
                    "WHERE dc.subcategory_id = ?\n" +
                    "LIMIT ?, 9"}, [subCategory, curRes]);

            res.dish = dish;
        } catch (e) {
            console.log('Error loading dishes from subcategory ' + subCategory + ' for page ' + page);
            console.log(e);
        }
        return res;
    };
