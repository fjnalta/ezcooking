...
class DishHandler {
    ...
    getDishes = async (id) => {
        let res;
        try {
            res = await this.dbHandler.query({sql: "SELECT * FROM dish WHERE creator_id = ?"}, id);
        } catch (e) {
    ...
    createDish = async (recipeName, duration, shortDescription, filename, description, user, category, subCategory, ingredients, userId, categoryId, subCategoryId) => {
        let dishQuery;
        let date = Date.now();
    ...
    updateDish = async (recipeName, duration, shortDescription, filename, description, user, category, subCategory, ingredients, dishId, userId, categoryId, subCategoryId) => {
        let date = Date.now();
    ...
    deleteDish = async (userId, dishId) => {
        let res;
        try {
            res = await this.dbHandler.query({sql: "SELECT creator_id, image_name FROM dish WHERE id = ?"}, dishId);
        } catch (e) {
    ...
    getUnitId = async (name) => {
        let res;
        try {
            res = await this.dbHandler.query({sql: "SELECT id FROM category_unit WHERE name = ?"}, name);
            return res[0].id;
        } catch (e) {
